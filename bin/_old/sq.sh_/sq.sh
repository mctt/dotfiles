...
while IFS= read -r path; do
    FOLDERS+=("$path")
done < <(sqlite3 "$DB_FILE" "SELECT path FROM items WHERE type='FOLDER' AND ($WHERE_CLAUSE);")
...
while IFS= read -r path; do
    FILES+=("$path")
done < <(sqlite3 "$DB_FILE" "SELECT path FROM items WHERE type='FILE' AND ($WHERE_CLAUSE);")
...
if [ ${#FOLDERS[@]} -gt 0 ]; then
    ...
    for folder in "${FOLDERS[@]}"; do
        ...
        if [ ! -d "$folder" ]; then
            echo "WARNING: Folder no longer exists: $folder (index may be outdated)"
            continue
        fi
...
if [ ${#FILES[@]} -gt 0 ]; then
    ...
    for file in "${FILES[@]}"; do
        ...
        if [ ! -f "$file" ]; then
            echo "WARNING: File no longer exists: $file (index may be outdated)"
            continue
        fi
