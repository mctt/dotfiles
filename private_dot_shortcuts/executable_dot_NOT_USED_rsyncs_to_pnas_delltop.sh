...
#works fine# ssh -i ~/storage/downloads/id_rsa -p 2218 user@v.corruptive.co.uk "screen -dmS rsyncs rsyncs_to_pnas.sh"
...
#2
#rsyncxxx_to_pnas.sh
ssh -i ~/storage/downloads/id_rsa -p 2218 user@v.corruptive.co.uk "screen -dmS rsyncs bash -c 'rsyncs_to_pnas.sh ; exit'" && sleep 2 && ssh -i ~/storage/downloads/id_rsa -p 2218 user@v.corruptive.co.uk "tail -f rsync.log 2>/dev/null || (sleep 3 && tail -f rsync.log)"
